...
{
    FILE *file = fopen("D:\\GitHub Desktop\\C_CodeLab\\Carros.txt", "r");
    if (file == NULL)
    {
...
{
    FILE *file = fopen("D:\\GitHub Desktop\\C_CodeLab\\Carros.txt", "w");
    if (file == NULL)
    {
    ...
    for (int i = 0; i < lista->id; ++i)
    {
        ...
        fprintf(file, "%d %s %s %d %s %.2f %.2f %s %c\n",
                p->id, p->marca, p->modelo, p->ano,
                p->cor, p->quilometragem,
                p->valor, p->categoria,
                p->disponibilidade);
    }
...
{
    if (lista == NULL || posicao < 0 || posicao > lista->id || lista->id >= TAM)
    {
        printf("A lista nao foi criada ou posicao invalida\n");
        return 0;
    }
